...
- a **boss**, carrying its display name, the game's own numeric identifier, its
  race, its level, whether it is epic, its position on the world map and in
  world coordinates, and its respawn information;
...
fractional precision to at least four decimal places, because source chances
such as `37.7861` are meaningful at that resolution.
...
Every boss and every item SHALL carry a grade. Rather than defaulting to the
weakest for 890 records, a grade SHALL be derived where the catalogue already
implies one:
...
every boss that names it — the label belongs to the weapon, not to each of the
120 times a boss mentions one, and is where a translation would go. A boss with
...
rather than only as the plan's presence. Fifteen of the locations have a plan
and the rest do not, and which is which SHALL be answerable from the list —
