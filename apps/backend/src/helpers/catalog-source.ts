 ...
 * `apps/frontend/mocks/raid-bosses/data.ts` and turns it into the shapes the
 ...
 * is input to the seed and only that. It cannot be `import`ed from here — it
 * belongs to another package and is TypeScript — so it is read as text, its
 * `import type` line and type annotations stripped, and evaluated. Fragile in
 * principle, but the file is generated by `mocks/raid-bosses/fetch.js` and its
 * shape changes only when that script does.
 *
...
const SOURCE_RELATIVE = path.join('apps', 'frontend', 'mocks', 'raid-bosses', 'data.ts');
